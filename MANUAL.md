...
1. 上のURLを開く
2. 左側に**図面のPDFを落とす**
3. **入力欄が自動で埋まります。** 埋まった欄は**黄色**になります
   （文字を手で貼り付けた場合は「この文字から入力欄を埋める」を押してください）
4. **黄色の欄を図面と見比べて確認する。** とくに**金額・面積・築年月**は
   読み取りを間違えることがあります。直すと黄色が消えます
5. 埋まらなかった欄を入力する
   （**物件番号は自動で採番されます。** `CMP-260805-01` のように
   日付＋連番が入り、1件追加するごとに次の番号に進みます）
6. 間違いがあれば**その場で赤く出ます**。直してから次へ進みます
7. **「一覧に追加」**を押す。同じ建物の別区画なら、物件番号と階数・面積だけ
   入れ直せば続けて追加できます
8. 全部入れ終わったら**「CSVを書き出す」**
9. 書き出したCSVを GitHub の `data/import/` に置き、
   Actionsの**「物件をCSVから一括登録」**を実行する
   （まず「確認だけ」で実行して、内容を見てから本番実行してください）
10. **登録が済んだら、`data/import/` に置いたCSVを削除する**

#### 気をつけること

- **PDFはこの画面の中だけで読み込まれます。** どこかへ送信されることはありません
- **自動入力は「下書き」です。** 図面の書き方は会社ごとに違うため、
  読み間違えることがあります。**黄色の欄は必ず図面と見比べてください**
- **読み取れなかったものは空欄のまま**にしています。
  それらしい値を入れて間違えるより安全なためです。画面下に理由が出ます
  （例：賃料が坪単価で書かれていた、築年に月が無かった）
...
- **物件番号は、公開したあとは変えないでください。**
  番号がそのままURLになるため、変えるとお客様のお気に入りや
  共有されたリンクが切れます
- 物件番号は書き換えられますが、**同じ日に別の人が同時に作業すると
  番号が重なることがあります**（この画面からmicroCMSの下書きは見えないため）。
  一括登録の「確認だけ」で番号を確かめてから本番実行してください
...
### 公開してしまった物件を、下書きに戻す（掲載をやめる）

下書きのつもりが公開状態になってしまったときは、**管理画面の操作だけで戻せます。**
GitHubもAPIキーも必要ありません。

1. 管理画面で、その物件を開く
2. 右上の **「公開終了」** を押す

**15分以内にサイトから消えます。** サイトへの取り込みは、microCMSで
公開中のものだけを読むためです。

⚠️ 消えるのは**サイトへの掲載だけ**です。microCMS の中身は残るので、
あとから「公開」を押せば元どおり出ます。**削除ではありません。**

全部の物件を下書きに戻しても大丈夫です。その場合サイトは
「該当する物件はありません」の表示になり、あとから戻せます。

#### 件数が多いとき（任意）

何十件もあって1件ずつが大変なときは、まとめて戻すこともできます。
**使わなくても構いません。** 上の「公開終了」を繰り返せば同じ結果です。

1. microCMS の **サービス設定 → 管理APIキー** でキーを作る
   （物件の登録に使う「APIキー」とは**別のもの**です）
2. Settings → Secrets and variables → Actions で
   `MICROCMS_MANAGEMENT_API_KEY` に登録する
3. Actionsタブ → **「物件をまとめて下書きに戻す」** → Run workflow
   - 対象: CSVのパス、または物件番号をカンマ区切りで
     （取り込みに使ったCSVが無ければ `data/properties.csv` が使えます）
   - まず **「確認だけ」** で対象を見てから本番実行
4. 終わったら Secrets のキーを削除する
