...
### 公開してしまった物件を、まとめて下書きに戻す

下書きのつもりが公開状態で登録してしまったときは、**まとめて戻せます。**
...
3. Actionsタブ → **「物件をまとめて下書きに戻す」** → Run workflow
   ...
   - まず **「確認だけ」** で実行し、対象を見てから本番実行
4. 終わったら Secrets のキーを削除する

下書きに戻すと、**15分以内にサイトから消えます**（サイトへの取り込みは
公開済みのものだけを読むため）。

⚠️ 消えるのは**サイトへの掲載だけ**です。microCMS の中身は残るので、
あとから公開し直せます。削除ではありません。

数件だけなら、管理画面で1件ずつ「公開終了」しても同じです。
