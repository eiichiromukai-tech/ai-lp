/* =====================================================
   物件データの検証と変換（サイトと運用ツールの共通ルール）
   -----------------------------------------------------
   ここが唯一の検証ルールです。次の3か所から同じものが使われます。

     tools/lib/schema.js  … CSVの読み書き（Node）
     tools/fetch-cms.js   … microCMSからの取り込み（Node）
     import.html          … 図面から下書きを作る画面（ブラウザ）

   マスタ（物件種別・エリア・こだわり条件）は data/properties.js から
   渡してもらいます。このファイル自身はファイルを読みません。
   ===================================================== */
(function (root, factory) {
  if (typeof module === 'object' && module.exports) module.exports = factory();
  else root.SCHEMA_CORE = factory();
}(typeof self !== 'undefined' ? self : this, function () {
  'use strict';

  /* マスタを渡すと、そのマスタに沿った検証・変換の一式を返す */
  function create(MASTERS) {
    ...
    /* 図面や募集資料に書かれている交通の文章を、路線・駅・徒歩分に分解する。
       ...
       microCMSへの一括登録（tools/import-cms.js）と、
       図面の下書き画面（js/import-page.js）の両方がこれを使う。 */
